...
raw_task_data <- read.table("11-11-25-vert-trans-no-allow-evol/data/VerticalTransTransEvol.dat", h=T) |>
  mutate(treatment = sub("^Parasites-", "", treatment)) # dont worry about the .dat filename we messed up but this is correct

steal_data <- read.table("11-11-25-vert-trans-no-allow-evol/data/VerticalTransTransEvolSymInst.dat", h=T) |>
